...
import { ExposedPort, GitpodPortsService, isExposedPort } from "./gitpod-ports-service";
...
export class GitpodPortViewContribution extends AbstractViewContribution<GitpodPortViewWidget> implements FrontendApplicationContribution {
    ...
    protected init(): void {
        ...
        this.portsService.onDidExposePort(port => this.handleDidExposePort(port));
    }
    ...
    protected async handleDidExposePort(port: ExposedPort): Promise<any> {
    ...
    protected async showOpenServiceNotification(port: ExposedPort, offerMakePublic = false): Promise<void> {
